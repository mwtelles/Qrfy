    ...
    <form action="" method="post" role="form">
        ...
        <?php if(count($data->domains) && ($this->settings->stores->domains_is_enabled || $this->settings->stores->additional_domains_is_enabled)): ?>
            <div class="form-group">
                ...
                <select id="domain_id" name="domain_id" class="form-control">
                    ...
                    <?php foreach($data->domains as $row): ?>
                        <option value="<?= $row->domain_id ?>" data-type="<?= $row->type ?>"><?= $row->url ?></option>
                    <?php endforeach ?>
                </select>
            ...
            <div id="is_main_store_wrapper" class="custom-control custom-switch my-3">
                <input id="is_main_store" name="is_main_store" type="checkbox" class="custom-control-input">
            ...
            <div <?= $this->user->plan_settings->custom_url_is_enabled ? null : 'data-toggle="tooltip" title="' . $this->language->global->info_message->plan_feature_no_access . '"' ?>>
                <div class="<?= $this->user->plan_settings->custom_url_is_enabled ? null : 'container-disabled' ?>">
                    <div class="form-group">
                        ...
                        <input type="text" id="url" name="url" class="form-control" value="" placeholder="<?= $this->language->store->input->url_placeholder ?>" />
        ...
        <div class="form-group">
            ...
            <input type="text" id="name" name="name" class="form-control" value="" placeholder="<?= $this->language->store->input->name_placeholder ?>" required="required" />
        </div>
        ...
        <div class="form-group">
            ...
            <input type="text" id="currency" name="currency" class="form-control" value="" required="required" />
        ...
        <div class="form-group">
            ...
            <select id="timezone" name="timezone" class="form-control">
                <?php foreach(DateTimeZone::listIdentifiers() as $timezone) echo '<option value="' . $timezone . '" ' . '>' . $timezone . '</option>' ?>
            </select>
